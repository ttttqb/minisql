...
{
public:
	interpreter(bufmgr &s) :Response(s){}
	void execfile(string filename);
	attribute parseattribute(string data);
...
attribute interpreter::parseattribute(string data) {
	char *c;
	if (data[0] == '\'') {
		string res;
...
void interpreter::parse(string input) {
	...
	if (operation == "execfile") {
		...
		while (1) {
			string data;
			str >> data;
			entry.push_back(parseattribute(data));
			str >> useless;
			if (useless == ")")
				break;
		}
		...
		while (str >> useless) {
			...
			fitter.addRule(Rule(index, op, parseattribute(data)));
		}
...
void interpreter::ipAddLine(string now) {
